...
const CandidateCheck = () => {
  ...
  const { message, hasDuplicates, candidates, recommendations } = useSelector(
    (state) => state.candidate
  );
  const [checkCandidate, { error: apiError, isLoading: apiLoading }] =
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await checkCandidate(formData).unwrap();
      ...
      setFormError(
        err?.data?.message || "Failed to check candidate. Please try again."
      );
    } finally {
      setIsChecking(false);
    }
  ...
  const generateWhatsAppLink = (hr, candidateName) => {
    ...
    const message = `Hi ${name}! I'm from TechCorpA. I see we both have offers for ${
      candidateName ?? "the candidate"
    }. Can we coordinate timing?`;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end">
            <button
              ...
              className="btn-blue px-6 py-2 disabled:opacity-50"
            >
              {isBusy ? (
                <>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {candidateData.candidate.currentRole} at{" "}
                        {candidateData.candidate.currentCompany}
                      </p>
                      <p className="text-sm text-gray-500 mt-1">
                        Experience: {candidateData.candidate.totalExperience}{" "}
                        year(s)
                      </p>
                      <p className="text-sm text-gray-500">
                        Location: {candidateData.candidate.location.city},{" "}
                        {candidateData.candidate.location.state}
                      </p>
                    </div>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-500">
                            ...
                            Variable:{" "}
                            {formatCurrency(offer.compensation.variable)}
                          </p>
                        </div>
                ...
                <div className="space-y-3">
                  {recommendations.map((recommendation, index) => (
                    ...
                    >
                      ...
                      {recommendation.action === "WHATSAPP_COORDINATION" &&
                        recommendation.hrContacts?.length > 0 && (
                      ...
                      {recommendation.action === "COMPETITIVE_OFFER" && (
                        <button
                          onClick={() =>
                            navigate(
                              `/candidate/${candidateData.candidate.id}/offer`
                            )
                          }
