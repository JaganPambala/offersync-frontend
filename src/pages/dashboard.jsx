...
} from "../redux/api/dashboard.apiSlice";
...
const Dashboard = () => {
  ...
  const stats = [
    ...
    },
    {
      name: "Success Rate",
      value:
        overview.successRate !== undefined ? `${overview.successRate}%` : "-",
      icon: CheckCircle,
      color: "green",
    },
    {
  ...
  if (error) {
    ...
    return (
      <div className="p-8 text-center text-red-600">
        Error loading dashboard: {error.data.error || "Unknown error"}
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {quickActions.map((action) => (
            <Link
              key={action.name}
              to={action.href}
              className="relative group bg-white p-6 focus-within:ring-2 focus-within:ring-inset focus-within:ring-blue-500 hover:bg-gray-50 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors"
            >
              <div>
                <span
          ...
          <div className="space-y-4">
            <div className="flex justify-between">
              ...
              <span className="text-sm font-medium text-gray-900">
                {whatsappMetrics.totalCommunications ?? "-"}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-500">Response Rate</span>
              <span className="text-sm font-medium text-gray-900">
                {whatsappMetrics.resolutionRate ?? "-"}%
              </span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className="bg-whatsapp-500 h-2 rounded-full"
                style={{ width: `${whatsappMetrics.resolutionRate ?? 0}%` }}
              ></div>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-500">Avg Response Time</span>
              <span className="text-sm font-medium text-gray-900">
                {whatsappMetrics.averageResponseTime ?? "-"} min
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-500">Collab Success Rate</span>
              <span className="text-sm font-medium text-gray-900">
                {whatsappMetrics.collaborationSuccessRate ?? "-"}%
              </span>
            </div>
          </div>
        </div>
                      ...
                      <div>
                        <span
                          className={`h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white ${
                            activity.type === "OFFER_CREATED"
                              ? "bg-blue-500"
                              : activity.type === "WHATSAPP_RESOLVED"
                              ? "bg-whatsapp-500"
                              : "bg-green-500"
                          }`}
                        >
                          ...
                          {activity.type === "CANDIDATE_ACCEPTED" && (
